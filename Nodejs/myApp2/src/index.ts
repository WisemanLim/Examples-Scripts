 ...
 * Send test data.
 */
async function testString(str: string): Promise<void> {
  const data = `${str}\r\n`;
  if (echoCheckbox.checked) {
    term.write(data);
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  // Test Send : data write in serial
  const testSend = document.getElementById('test') as HTMLSelectElement;
  testSend.addEventListener("click", function() { testString("Hello"); });
